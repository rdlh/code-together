var FirepadUserList = (function() {
  function FirepadUserList(ref, place, userId) {
    ...
    var self = this;
    this.displayName_ = 'Membre ' + Math.floor(Math.random() * 100);
  ...
  FirepadUserList.prototype.makeUserEntryForSelf_ = function() {
    ...
    var nameHint = elt('div', 'VOTRE NOM ICI', { 'class': 'firepad-userlist-name-hint'} );

    // Update Firebase when name changes.
    on(nameInput, 'change', function(e) {
      var name = nameInput.value || "Membre " + Math.floor(Math.random() * 100);
